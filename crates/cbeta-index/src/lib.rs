...
//! via atomic `{dir}.tmp` → rename (unique sibling on rebuild). Search/get product wiring
//! lives elsewhere (L3).
...
pub use publish::{publish_dir, resolve_publish_dest, stage_tmp_dir, write_current_pointer};
...
pub use writer::{
    ...
    write_atomic_dir, write_tantivy_dir, IndexableLine,
};
